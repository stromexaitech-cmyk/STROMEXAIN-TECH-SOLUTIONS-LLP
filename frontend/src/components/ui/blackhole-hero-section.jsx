...
/**
 * @typedef {Object} BlackHoleHeroSectionProps
 * @property {number} [distance] Camera distance from the hole, in horizon radii.
 * @property {number} [elevation] How far the camera sits above the disc, in degrees. Near 0 the
 *   disc is edge-on and the far side arches over the shadow as a halo — the shot everyone knows.
 *   Past about 25 the halo folds away and you are looking down at a ring instead.
 * @property {number} [azimuth] Where the camera sits around the hole, in degrees.
 * @property {number} [orbitSpeed] Degrees the camera swings around the hole each second. 0 by
 *   default, and that is the point: the camera holds still and the gas does the moving. Turn it
 *   up and the whole frame drifts, which pulls the eye off the rings.
 * @property {number} [roll] Turns the picture about the line of sight, in degrees, so the disc
 *   runs on a diagonal.
 * @property {number} [fov] Vertical field of view, in degrees.
 * @property {number} [diskInner] Inner edge of the disc, in horizon radii. 3 is the innermost
 *   stable orbit; below it gas falls in.
 * @property {number} [diskOuter] Outer edge of the disc.
 * @property {number} [diskThickness] Half-thickness of the disc at its inner edge. It flares
 *   outward from there.
 * @property {number} [diskDensity] How much gas there is. Above 1 the near side starts to hide
 *   the far side.
 * @property {number} [brightness] Overall brightness of the gas before tone mapping.
 * @property {number} [spinSpeed] Turns of the inner rim per second. The rest of the disc follows
 *   Kepler, so it lags — hard. This is where nearly all the movement in the picture comes from:
 *   the rim is imaged over and over into the rings that hug the shadow, and those rings carry
 *   the fastest gas in the frame.
 * @property {number} [grain] Size of the turbulence in the gas. Higher is finer.
 * @property {number} [doppler] Relativistic beaming, 0 to 1. The gas at the inner rim runs at
 *   0.4c, so the side coming at you is thrown brighter and bluer and the receding side is
 *   dimmed — physically it is g³, and this dials the exponent. The film everyone has seen set
 *   this to 0 to keep the ring even; 1 is the truth.
 * @property {string} [hotColor] Brightest gas, at the inner rim.
 * @property {string} [midColor] Mid-disc.
 * @property {string} [coolColor] Coolest gas, at the outer edge.
 * @property {number} [starBrightness] Background stars, 0 to 2. Off by default. They are lensed
 *   along with everything else, so near the rim they smear into arcs — which looks right in a
 *   still and busy in motion, and the gas is what should be moving.
 * @property {number} [glow] Bloom, 0 to 2.
 * @property {number} [exposure] Exposure into the tone curve.
 * @property {number} [vignette] Corner darkening, 0 to 1.
 * @property {number} [steps] Steps each ray may take. This is the one dial that costs real time.
 *   300 is clean; 180 will do on a phone; below 140 the disc starts to band.
 * @property {number} [resolution] Render scale, 0.5 to 1. Drop it before you drop `steps`.
 * @property {number} [maxDpr] Cap on device pixel ratio.
 * @property {[number, number]} [focus] Where the hole sits in the frame, as fractions of width
 *   and height, top left origin. [0.5, 0.5] centres it. The default puts it high and right, which
 *   is where the film frames it and which leaves the reading half of a hero clear.
 * @property {"none"|"left"|"right"|"top"|"bottom"} [scrim] Lays a veil over one edge so text can
 *   sit there and still be read.
 * @property {number} [scrimStrength] How dark that edge gets, 0 to 1.
 * @property {boolean} [paused] Freeze on the current frame.
 */
...
/**
 * @param {BlackHoleHeroSectionProps & React.HTMLAttributes<HTMLDivElement>} props
 */
export function BlackHoleHeroSection({
  ...
  // Kept current outside of render (in an effect, not inline) so the
  // long-lived WebGL effect below can always read fresh prop values without
  // needing to restart the whole render loop on every prop change.
  useEffect(() => {
    props.current = {
      distance, elevation, azimuth, orbitSpeed, roll, fov, diskInner, diskOuter,
      diskThickness, diskDensity, brightness, spinSpeed, grain, doppler, hotColor,
